...
$sql = "SELECT `Email`,`Password`,`Collno`,`Collname` FROM `regop` WHERE `Email`= ?";
...
$stmt->bind_result($email, $db_password, $collno, $collname);
$stmt->fetch();
...
$now = time();
$mysqli->query("INSERT INTO loginattempts(email, time) VALUES ('$email', '$now')");
return false;
}
