...
<?php foreach ($data as $sheetIndex => $sheet): ?>
    ...
    <form id="save-form-<?php echo $sheetIndex; ?>">
        ...
        <tr>
            <?php for ($col = 0; $col < count($sheet['data'][$i]); $col++): ?>
                <?php if ($col >= 0 && $col < 4): ?>
                    ...
                    <td>
                        <input type="text" 
                               name="data_col_<?php echo $col + 1; ?>[]" 
                               value="<?php echo htmlspecialchars($sheet['data'][$i][$col]); ?>" 
                               class="form-control">
                    </td>
                <?php else: ?>
                    <!-- Non-Editable Columns -->
                    <td><?php echo htmlspecialchars($sheet['data'][$i][$col]); ?></td>
                <?php endif; ?>
            <?php endfor; ?>
            <!-- Save Button -->
            <td><button type="button" class="btn btn-primary save-btn" data-index="<?php echo $sheetIndex; ?>">Save</button></td>
        </tr>
    <?php endfor; ?>
</tbody>
...
<script>
    $(document).ready(function() {
        $('.save-btn').click(function() {
            ...
            let formData = $(`#save-form-${index}`).serialize();
